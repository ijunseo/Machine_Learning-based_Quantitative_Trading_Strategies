...
class CPCVSplitter:
    ...
    def split(
        self, X: pd.DataFrame, barrier_times: Optional[pd.DataFrame] = None
        ...
        for test_blocks in combinations(range(self.n_blocks), self.n_test_blocks):
            ...
            if barrier_times is not None:
                train_idx = apply_purging(
                    train_idx=train_idx, test_idx=test_idx, barrier_times=barrier_times
                )
            else:
                # 時間ベースのPurging
                train_idx = apply_time_based_purging(
                    train_idx=train_idx, test_idx=test_idx, purge_window=self.purge_window
                )

            # Embargo適用
            train_idx = apply_embargo(
                train_idx=train_idx, test_idx=test_idx, embargo_window=self.embargo_window
            )
...
def apply_purging(
    train_idx: np.ndarray, test_idx: np.ndarray, barrier_times: pd.DataFrame
) -> np.ndarray:
    """
...
def apply_time_based_purging(
    train_idx: np.ndarray, test_idx: np.ndarray, purge_window: int
) -> np.ndarray:
    """
...
def apply_embargo(train_idx: np.ndarray, test_idx: np.ndarray, embargo_window: int) -> np.ndarray:
    """
    Embargoを適用する。
