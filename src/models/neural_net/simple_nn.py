...
class SimpleNNArchitecture(nn.Module):
    ...
    def __init__(
        self, input_dim: int, hidden_dims: list[int], output_dim: int, dropout: float = 0.3
    ) -> None:
        super().__init__()
...
class SimpleNN(BaseModel):
    ...
    def fit(
        ...
        # データローダー作成
        train_dataset = TensorDataset(torch.FloatTensor(X_train), torch.LongTensor(y_train_shifted))
